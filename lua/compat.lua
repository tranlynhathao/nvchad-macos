...
if vim.g.noah_compat_loaded then return M end
...
if vim.fn.has "nvim-0.12" == 1 then
  ...
  local function unwrap_ts_node(value)
    ...
    if type(first) == "table" and type(first.range) == "function" then return first end

    return value
  end

  local function normalize_diagnostic_filter(value, namespace)
    if type(value) == "number" then return namespace and { bufnr = value, ns_id = namespace } or { bufnr = value } end

    if value == nil then return namespace and { ns_id = namespace } or nil end

    if type(value) == "table" then
      if namespace ~= nil and value.ns_id == nil then return vim.tbl_extend("keep", value, { ns_id = namespace }) end
      return value
    end
  ...
  vim.validate = function(name, value, validator, optional, message)
    if validator ~= nil or type(name) ~= "table" then return original_validate(name, value, validator, optional, message) end
    ...
    for _, param_name in ipairs(keys) do
      ...
      if type(spec) ~= "table" then error(string.format("opt[%s]: expected table, got %s", param_name, type(spec)), 2) end

      if type(spec[2]) == "string" and legacy_validators[spec[2]] ~= nil then spec[2] = legacy_validators[spec[2]] end
  ...
  if not vim.g.noah_diagnostic_compat then
    vim.diagnostic.enable = function(enable, filter)
      if type(enable) == "boolean" then return original_diagnostic_enable(enable, filter) end
    ...
    if type(vim.diagnostic.disable) ~= "function" then
      vim.diagnostic.disable = function(filter, namespace) return original_diagnostic_enable(false, normalize_diagnostic_filter(filter, namespace)) end
    end
  ...
  ts.get_node_range = function(node_or_range) return original_get_node_range(unwrap_ts_node(node_or_range)) end

  ts.get_range = function(node, source, metadata) return original_get_range(unwrap_ts_node(node), source, metadata) end

  ts.get_node_text = function(node, source, opts) return original_get_node_text(unwrap_ts_node(node), source, opts) end

  ts.is_ancestor = function(dest, source) return original_is_ancestor(unwrap_ts_node(dest), unwrap_ts_node(source)) end

  ts.node_contains = function(node, range) return original_node_contains(unwrap_ts_node(node), range) end

  ts.is_in_node_range = function(node, line, col) return original_is_in_node_range(unwrap_ts_node(node), line, col) end
end
...
if not vim.g.noah_lsp_float_defaults then
  ...
  vim.lsp.buf.hover = function(config) return original_hover(vim.tbl_deep_extend("force", { border = border }, config or {})) end

  vim.lsp.buf.signature_help = function(config) return original_signature_help(vim.tbl_deep_extend("force", { border = border }, config or {})) end
end
...
if not vim.g.noah_tsdk_compat then
  ...
  local function valid_tsdk(path)
    if type(path) ~= "string" or path == "" or vim.fn.isdirectory(path) ~= 1 then return false end
  ...
  local function resolve_typescript_tsdk(root_dir)
    if valid_tsdk(vim.g.tsdk) then return vim.g.tsdk end

    local ok, util = pcall(require, "lspconfig.util")
    if ok then
      local candidate = util.get_typescript_server_path(type(root_dir) == "string" and root_dir or vim.uv.cwd())
      if valid_tsdk(candidate) then return candidate end
    end

    if valid_tsdk(mason_tsdk) then return mason_tsdk end
  end
  ...
  local function ensure_typescript_tsdk(config)
    ...
    if type(init_options) ~= "table" or type(init_options.typescript) ~= "table" then return config end

    if valid_tsdk(init_options.typescript.tsdk) then return config end
  ...
  local function wrap_before_init(config)
    if type(config) ~= "table" or type(config.before_init) ~= "function" or config._noah_tsdk_before_init_wrapped then return config end
